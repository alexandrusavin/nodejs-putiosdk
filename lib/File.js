            ...
            try {
                ...
                if(_.isEmpty(route.returns) || typeof cb !== 'function') {
                    return new Promise((resolve, reject) => {
                        const request = this.client.request(apiPath,
                            {form: params, followRedirect: false}, method, true);

                        request
                            .on('response', incommingMessage => {
                                const response = getStream(incommingMessage);
                                response.then(
